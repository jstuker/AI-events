...
describe("EventDetailsSection", () => {
  ...
  it("renders event URL input", () => {
    render(<EventDetailsSection {...defaultProps()} />);
    expect(screen.getByLabelText("Event URL")).toHaveValue(
  ...
  it("renders description textarea", () => {
    ...
    expect(screen.getByLabelText("Description")).toHaveValue(
      "A test event for AI enthusiasts",
    );
  });
  ...
  describe("user interactions", () => {
    ...
    it("calls setField when attendance mode changes", async () => {
      ...
      await userEvent.selectOptions(
        screen.getByLabelText("Attendance Mode"),
        "online",
      );
  ...
  describe("validation errors", () => {
    it("displays event name error", () => {
      render(
        <EventDetailsSection
          {...defaultProps({ errors: { event_name: "Name is required" } })}
        />,
      );
    ...
    it("displays slug error", () => {
      render(
        <EventDetailsSection
          {...defaultProps({ errors: { slug: "Invalid slug" } })}
        />,
      );
    ...
    it("displays event URL error", () => {
      render(
        <EventDetailsSection
          {...defaultProps({ errors: { event_url: "Invalid URL" } })}
        />,
      );
    ...
    it("displays start date error", () => {
      render(
        <EventDetailsSection
          {...defaultProps({ errors: { event_start_date: "Date required" } })}
        />,
      );
    ...
    it("displays end date error", () => {
      render(
        <EventDetailsSection
          {...defaultProps({ errors: { event_end_date: "Invalid date" } })}
        />,
      );
    ...
    it("applies error border class to name input", () => {
      render(
        <EventDetailsSection
          {...defaultProps({ errors: { event_name: "required" } })}
        />,
      );
