...
describe("fetchEventByPath", () => {
  it("fetches and parses event file by path", async () => {
    ...
    const result = await fetchEventByPath(
      "token",
...
describe("fetchEventById", () => {
  ...
  it("throws when event not found", async () => {
    ...
    await expect(fetchEventById("token", "evt-999")).rejects.toThrow(
      "Event not found: evt-999",
    );
  });
});
...
describe("fetchEventHistory", () => {
  it("maps commit entries", async () => {
    ...
    const history = await fetchEventHistory(
      "token",
      "content/events/2026/03/15/evt-001.md",
    );
...
describe("validateEventPath", () => {
  it("accepts valid event file paths", () => {
    expect(() =>
      validateEventPath("content/events/2026/03/15/evt-001.md"),
    ).not.toThrow();
  });

  it("rejects paths with directory traversal (..)", () => {
    expect(() => validateEventPath("content/events/../../.env")).toThrow(
      "path traversal not allowed",
    );
  });

  it("rejects paths not starting with content/events/", () => {
    expect(() => validateEventPath("package.json")).toThrow(
      "must start with content/events/",
    );
  });

  it("rejects paths like content/events/../../.env after prefix", () => {
    expect(() =>
      validateEventPath("content/events/../../../package.json"),
    ).toThrow("path traversal not allowed");
  });

  it("rejects absolute paths", () => {
    expect(() => validateEventPath("/etc/passwd")).toThrow(
      "must start with content/events/",
    );
  });

  it("rejects paths without proper date structure", () => {
    expect(() => validateEventPath("content/events/file.md")).toThrow(
      "must match pattern",
    );
  });
});
