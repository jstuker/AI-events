...
export async function updateFileContent(
  ...
  if (!response.ok) {
    if (response.status === 409) {
      throw new Error('Conflict: the file has been modified by another user. Please reload and try again.')
    }
    throw new Error(`GitHub API error: ${response.status} ${response.statusText}`)
  }
...
export async function getFileCommits(
  ...
  return fetchJson<readonly GitHubCommitEntry[]>(
    repoUrl(`/commits?path=${encodeURIComponent(path)}&sha=${GITHUB_CONFIG.branch}`),
    token,
  )
}
