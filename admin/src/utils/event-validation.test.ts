...
describe("validateEvent", () => {
  ...
  it("accepts valid email with subdomain", () => {
    const form = createFormData({ contact_email: "first.last@sub.domain.org" });
    const errors = validateEvent(form);
    expect(errors.contact_email).toBeUndefined();
  });
  ...
  it("rejects email with empty domain label (user@.com)", () => {
    const form = createFormData({ contact_email: "user@.com" });
    const errors = validateEvent(form);
    expect(errors.contact_email).toContain("valid email");
  });

  it("rejects email with consecutive dots (user@domain..com)", () => {
    const form = createFormData({ contact_email: "user@domain..com" });
    const errors = validateEvent(form);
    expect(errors.contact_email).toContain("valid email");
  });

  it("rejects email with empty local part (@domain.com)", () => {
    const form = createFormData({ contact_email: "@domain.com" });
    const errors = validateEvent(form);
    expect(errors.contact_email).toContain("valid email");
  });

  it("rejects email without domain (user@)", () => {
    const form = createFormData({ contact_email: "user@" });
    const errors = validateEvent(form);
    expect(errors.contact_email).toContain("valid email");
  });

  it("rejects email with domain starting with hyphen (user@-domain.com)", () => {
    const form = createFormData({ contact_email: "user@-domain.com" });
    const errors = validateEvent(form);
    expect(errors.contact_email).toContain("valid email");
  });
